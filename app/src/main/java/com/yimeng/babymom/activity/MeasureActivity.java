...
public class MeasureActivity extends BaseActivity implements AdapterView.OnItemClickListener, View.OnTouchListener {
    ...
    private final String[] FUNS_DES = new String[]{"宝宝血型", "预产期", "胎儿体重", "身高预测"};
    private int mCheckedItemPosition;
    private final int[] FUNS_PICS = new int[]{R.drawable.blood, R.drawable.date, R.drawable.weight, R.drawable.height};
    ...
    protected void setListener() {
        ...
        mPagerAdapter = new DefaultFragmentPagerAdapter(getSupportFragmentManager(), fragments);
    ...
    protected void initData() {
        mFunPicBeans.clear();
        // 默认选中第一个条目
        mFunPicBeans.add(new PicDesBean(FUNS_PICS[0], FUNS_DES[0], false));
        for (int i = 1; i < FUNS_DES.length; i++) {
            mFunPicBeans.add(new PicDesBean(FUNS_PICS[i], FUNS_DES[i]));
        }
    ...
    public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
        KeyBoardUtils.closeKeybord(viewPager, this);
        viewPager.setCurrentItem(position);
        tv_title.setText(FUNS_DES[position]);
